...
def trace_impact(args: dict) -> dict:
    ...
    Returns:
        dict with keys: node_id, upstream_callers, downstream_callees,
                        exceptions_raised, configs_read, error.
    """
    ...
    try:
        driver = get_driver()
        result = get_impact_analysis(driver, node_id, repo_id=repo_id)
        return {
            "node_id": node_id,
...
def get_call_chain(args: dict) -> dict:
    ...
    Args:
        args:
            node_id (str): Node ID in format "<file>::<name>"
            direction (str): "upstream" or "downstream". Default: "downstream"
            max_depth (int): Maximum traversal depth. Default: 4.
            repo_id (str, optional): Repo scope.

    Returns:
        dict with keys: node_id, direction, chain, error.
    """
    node_id = args.get("node_id", "")
    ...
    if direction not in ("upstream", "downstream"):
        return {
            ...
            "node_id": node_id, "direction": direction, "chain": [],
        }

    try:
        driver = get_driver()
        if direction == "upstream":
            chain = get_upstream_callers(driver, node_id, max_depth=max_depth, repo_id=repo_id)
        else:
            chain = _get_call_chain(driver, node_id, max_depth=max_depth, repo_id=repo_id)

        return {"node_id": node_id, "direction": direction, "chain": chain, "error": None}
    except Exception as e:
        return {"error": str(e), "node_id": node_id, "direction": direction, "chain": []}
